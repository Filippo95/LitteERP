...
<%@page import="BusinessLogic.VirtualEntities.VirtualDrain"%>
...
%>
<jsp:forward page="Drain.jsp"/>
<%
    } else {
        status = request.getParameter("status");
    }
    if (status.equals("insert")) {
        loggedon = DrainBean.isLoggedOn(cookies);
        status = "insertDrain";
    } else {
        if (status.equals("insertDrainRow")) {
            loggedon = DrainBean.insertRow(cookies);
            drain = DrainBean.getAll().get(0);
        }
        if (status.equals("insertDrain")) {
            loggedon = DrainBean.insertDrainAndRow(cookies);
            drain = DrainBean.getAll().get(0);
            status = "insertDrainRow";
        }
        if (status.equals("insertRowFromDetails")) {
            loggedon = DrainBean.isLoggedOn(cookies);
            drain = DrainBean.getAll().get(0);
            status = "insertDrainRow";
        }
...
<%
    ...
    products = DrainBean.getProducts();
%>
<!DOCTYPE html>
<html>
    <head>
        ...
        <script language="javascript">
            ...
            function selectedProduct(code,quantity)
            {
    ...
    <body>
        ...
        <div class="row">
            <div class="col-lg-12">
                <%if (DrainBean.getId() != 0) {%>
                <h3 class="page-header">Stai inserendo una riga riguardo alo scarico della data:</h3>
                <p><%=drain.getDdate()%></p>
                <%} else {%>
                <h3 class="page-header">Inserisci le righe dello scarico</h3>
                <%}%>
            </div>
        </div>
        <div class="row">
                        ...
                        <div class="row">
                            <div class="col-lg-6">
                                <form role="form" name="mainForm" action="" method="post">
                                    <input type="hidden" name="id" value="<%=DrainBean.getId()%>"/><br/>
                                        ...
                                        <table class="table table-striped table-bordered table-hover" id="table">
                                            ...
                                            <tbody >
                                                ...
                                                %><tr>
                                                    ...
                                                    <td><a class="btn btn-default btn-sm btn-block" href="javascript:selectedProduct('<%=products.get(i).getId()%>','<%=products.get(i).getCurrentlyPresentNumber()%>');">Seleziona</a></td>
                                                </tr><%
                                                    }%>
                                            </tbody>
                                        </table>
                                    ...
                                    <div class="form-group">
                                        <a class="btn btn-default btn-lg btn-block" href="javascript:submitForm('DrainRowInsert.jsp');">Inserisci Nuova Riga</a>
                                    </div>
                                    <div class="form-group">
                                        <a class="btn btn-warning" href="javascript:cancel();">Annulla</a>
                                        <a class="btn btn-primary" href="javascript:submitForm('DrainDetails.jsp');">Fine inserimento righe</a>
                                    </div>
                                </form>

                                <form name="cancelForm"  action="<%if(status.equals("insertDrain")){%>Drain.jsp<%}else{%>DrainDetails.jsp<%}%>" method="post">
                                    <input type="hidden" name="status" value="view"/>
                                    <input type="hidden" name="id" value="<%=DrainBean.getId()%>"/>
                                </form>
                            </div>
                        </div>
                    </div>
